...
public class MMPluginAPIImpl implements IMMPluginAPI {
    ...
    public void jumpToSettingView(String str, String str2) {
        a(str, ParamConst.USER_PROPERTIES_RSP_PROFILE, new Intent().putExtra(ConstantsUI.Contact.KUser, str2).setFlags(67108864));
    }
    ...
    public boolean registerQRCodePattern(String str, MMessage.CallBack callBack, String str2) {
        MMessage.Receiver receiver = this.bz;
        if (receiver != null) {
            this.R.unregisterReceiver(receiver);
        }
        MMessage.Receiver receiver2 = new MMessage.Receiver(callBack);
        this.bz = receiver2;
        this.R.registerReceiver(receiver2, new IntentFilter(MMPluginProviderConstants.PluginIntent.ACTION_QRCODE_SCANNED));
    ...
    public boolean sendMsgNotify(String str, String str2, int i, String str3, Class<?> cls) {
        ...
        return Util.nullAsFalse((Boolean) c("plugindb://" + this.aE + "/action.msgnotify?username=" + str + "&msgContent=" + str2 + "&msgType=" + i + "&customNotify=" + str3 + "&intentUri=" + Base64.encodeToString(new Intent(this.R, cls).toUri(0).getBytes(), false)));
    }
